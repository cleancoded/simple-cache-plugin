...
if ( ! function_exists('Cleancoded_serve_file_cache') ) {
	return;
}
...
if ( ! empty( $_COOKIE ) ) {
	...
	if ( ! empty( $_COOKIE['Cleancoded_commented_posts'] ) ) {
		foreach ( $_COOKIE['Cleancoded_commented_posts'] as $path ) {
...
if ( ! empty( $GLOBALS['Cleancoded_config']['advanced_mode'] ) && ! empty( $GLOBALS['Cleancoded_config']['cache_exception_urls'] ) ) {
	$exceptions = preg_split( '#(\n|\r)#', $GLOBALS['Cleancoded_config']['cache_exception_urls'] );

	$regex = ( ! empty( $GLOBALS['Cleancoded_config']['enable_url_exemption_regex'] ) ) ? true : false;

	foreach ( $exceptions as $exception ) {
		if ( Cleancoded_url_exception_match( $exception, $regex ) ) {
			// Exception match.
			return;
		}
	}
...
Cleancoded_serve_file_cache();

ob_start( 'Cleancoded_file_cache' );
