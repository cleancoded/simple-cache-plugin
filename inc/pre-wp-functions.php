 ...
 * @return string
 */
function Cleancoded_file_cache( $buffer, $flags ) {
	global $post;

	$cache_dir = Cleancoded_get_cache_dir();
	...
	if ( ! file_exists( $cache_dir ) ) {
		...
		$buffer = apply_filters( 'Cleancoded_pre_cache_buffer', $buffer );
	}

	$url_path = Cleancoded_get_url_path();
	...
	if ( ! empty( $GLOBALS['Cleancoded_config']['enable_gzip_compression'] ) && function_exists( 'gzencode' ) ) {
	...
	if ( function_exists( 'ob_gzhandler' ) && ! empty( $GLOBALS['Cleancoded_config']['enable_gzip_compression'] ) ) {
 ...
 * @return string
 */
function Cleancoded_get_url_path() {
 ...
 * @return string
 */
function Cleancoded_get_cache_path() {
	return rtrim( WP_CONTENT_DIR, '/' ) . '/cache/cleancoded-simple-cache';
}
 ...
 * @since 1.0
 */
function Cleancoded_serve_file_cache() {
	$cache_dir = ( defined( 'Cleancoded_CACHE_DIR' ) ) ? rtrim( Cleancoded_CACHE_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/cache/cleancoded-simple-cache';

	$file_name = 'index.html';

	if ( function_exists( 'gzencode' ) && ! empty( $GLOBALS['Cleancoded_config']['enable_gzip_compression'] ) ) {
		$file_name = 'index.gzip.html';
	}

	$path = $cache_dir . '/' . rtrim( Cleancoded_get_url_path(), '/' ) . '/' . $file_name;
 ...
 * @return string
 */
function Cleancoded_get_cache_dir() {
	return ( defined( 'Cleancoded_CACHE_DIR' ) ) ? rtrim( Cleancoded_CACHE_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/cache/cleancoded-simple-cache';
}
 ...
 * @return string
 */
function Cleancoded_get_config_dir() {
	return ( defined( 'Cleancoded_CONFIG_DIR' ) ) ? rtrim( Cleancoded_CONFIG_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/sc-config';
}
 ...
 * @return string
 */
function Cleancoded_get_config_file_name() {
 ...
 * @return bool|array
 */
function Cleancoded_load_config() {
	if ( @file_exists( Cleancoded_get_config_dir() . '/config-network.php' ) ) {
		return include Cleancoded_get_config_dir() . '/config-network.php';
	} elseif ( @file_exists( Cleancoded_get_config_dir() . '/' . Cleancoded_get_config_file_name() ) ) {
		return include Cleancoded_get_config_dir() . '/' . Cleancoded_get_config_file_name();
	}
 ...
 * @return boolean
 */
function Cleancoded_url_exception_match( $exception, $regex = false ) {
