...
class Cleancoded_Cron {
	...
	private function setup() {
		...
		add_action( 'Cleancoded_purge_cache', array( $this, 'purge_cache' ) );
	...
	public function filter_cron_schedules( $schedules ) {
		...
		$schedules['simple_cache'] = array(
			'interval' => apply_filters( 'Cleancoded_cache_purge_interval', $interval ),
			'display'  => esc_html__( 'Simple Cache Purge Interval', 'cleancoded-simple-cache' ),
		);
		return $schedules;
	}
	...
	public function unschedule_events() {
		$timestamp = wp_next_scheduled( 'Cleancoded_purge_cache' );

		wp_unschedule_event( $timestamp, 'Cleancoded_purge_cache' );
	}
	...
	public function schedule_events() {
		...
		if ( ! $timestamp ) {
			wp_schedule_event( time(), 'simple_cache', 'Cleancoded_purge_cache' );
		}
	}
